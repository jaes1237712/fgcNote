...
<script lang="ts">
	...
	let { allCharacters, characterMe, characterOpponent }: { 
	...
	$effect(()=>{
		if(!characterMe){moveImagesMe=[]}
		else{
			characterControllerFindImages({path:{id:characterMe.id}}).then((resp)=>{
				if(resp.data){moveImagesMe = resp.data}
			})
		}
		if(!characterOpponent){moveImagesOpponent=[]}
		else{
			characterControllerFindImages({path:{id:characterOpponent.id}}).then((resp)=>{
				if(resp.data){moveImagesOpponent = resp.data}
			})
		}
	})
...
<div class="search-character-image">
	<div class="character-versus">
		...
		<p>vs.</p>
		<select-character allCharacters={allCharacters} 
			defaultCharacter={characterOpponent}
			size="6vw" onselectCharacter={(event)=>{

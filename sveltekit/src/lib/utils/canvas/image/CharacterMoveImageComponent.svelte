<script lang="ts">
    ...
    let { layer, data, userSettings, stage }: { layer: Konva.Layer, data: CanvasCharacterMoveImageDto, userSettings: UserSettings, stage: Konva.Stage } = $props();
    const scale =
    ...
    onMount(() => {
        Konva.Image.fromURL(
		PUBLIC_NESTJS_URL + data.characterMoveImage.filePath,
		function (image) {
			image.setAttrs({
